...
/*模拟数据*/
 //var timecell = [ {
	...
	// "endTime": new Date().getTime() - 3 * 3600 * 1000,
	// "style": {
	//	 "background": "rgba(242,240,15, 0.498039)"
	// }
	// //}, {
...
function init(start_timestamp, timecell) {
    ...
    drawLine(canvansW / 2, 0, canvansW / 2, 33, "rgb(242,240,15", 2); //中间播放点时间线
    add_events();
...
function add_graduations(start_timestamp) {
    ...
    for (var i = 0; i < num_steps; i++) {
        ...
        if (date.getUTCHours() == 0 && date.getUTCMinutes() == 0) {
            ...
            ctx.fillStyle = "rgba(151,158,167,1)"; //时间颜色
        ...
        drawLine(graduation_left, 0, graduation_left, lineH, "rgba(242,240,15,1)", 1);
    }
}
...
function drawLine(beginX, beginY, endX, endY, color, width) {
    ...
    ctx.moveTo(beginX, beginY); //把画笔移动到指定的坐标
    ctx.lineTo(endX, endY); //绘制一条从当前位置到指定坐标(200, 50)的直线.
...
function mousedownFunc(e) {
    g_isMousedown = true;
    g_mousedownCursor = get_cursor_x_position(e) - 300; //记住mousedown的位置
}
...
function mousemoveFunc(e) {
    var pos_x = get_cursor_x_position(e) - 300;
    var px_per_ms = canvansW / (hours_per_ruler * 60 * 60 * 1000); // px/ms
